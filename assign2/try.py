...
def mark_matrix(mat):
    ...
    while (True in zero_bool_mat_copy):
        ...
        zero_index = np.where(zero_bool_mat_copy[min_row[1]] == True)[0][0]
        marked_zero.append((min_row[1], zero_index))
        zero_bool_mat_copy[min_row[1], :] = False
...
def adjust_matrix(mat, cover_rows, cover_cols):
    ...
    for row in range(len(cover_rows)):  
        for col in range(len(cover_cols)):
            mat[cover_rows[row], cover_cols[col]] = mat[cover_rows[row], cover_cols[col]] + min_num
    return mat
